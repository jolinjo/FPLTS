...
def validate_process_flow(sku: str, prev_station: str, curr_station: str) -> Tuple[bool, Optional[str]]:
    ...
    # 解析流程清單（例如 "P1, P2, P3, P4, P5"）
    flow_list = [station.strip() for station in flow_section.split(',')]
    
    # 檢查上一站是否在流程清單中
    if prev_station not in flow_list:
        return False, f"上一站 {prev_station} 不在 SKU {sku} 的流程清單中"
    
    # 取得上一站在流程清單中的索引
    prev_index = flow_list.index(prev_station)
    ...
    if curr_station != expected_next_station:
        # 跳站或錯誤的站點
...
def get_next_station(sku: str, current_station: str) -> Optional[str]:
    ...
    # 檢查當前站在流程清單中的位置
    if current_station not in flow_list:
        return None
    
    current_index = flow_list.index(current_station)
