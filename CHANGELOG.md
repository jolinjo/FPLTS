...
- **遷入條碼列表顯示**：
  ...
  - 只顯示有遷入記錄但尚未遷出的條碼
  - 每30秒自動更新列表
  - 顯示格式：第一行顯示型號和數量，第二行顯示工單和容器，右側顯示遷入時間和條碼
  - 按遷入時間由早到晚排序
- **自訂圖示支援**：
  - 遷入和遷出按鈕支援自訂圖片
  - 圖片檔案：`static/images/icons/inbound.png` 和 `outbound.png`
  - 圖示大小：128px（放大兩倍）
  - 如果圖片載入失敗，自動回退顯示 emoji

### 改進
- **UI 佈局優化**：
  - 遷入、站點、遷出三個按鈕高度統一（220px）
  - 按鈕樣式統一：使用 card glass 樣式，具有陰影和 hover 動畫效果
  - 使用柔和的漸層背景色，降低對比度
  - 操作員資訊與標題融合在同一行
  - 操作員卡片寬度調整，支援7位數字工號顯示
- **訊息顯示區域優化**：
  - 訊息顯示在標題和操作員之間
  - 使用絕對定位，不影響標題列高度
  - 從頂部滑入的動畫效果
  - 訊息寬度減少一半（最大300px），更緊湊
- **按鈕互斥邏輯**：
  - 遷出和首站遷出按鈕使用絕對定位重疊，確保互斥顯示
  - P1 站點顯示首站遷出，其他站點顯示貨物遷出
  - 使用 `display: none/flex` 控制顯示，不再使用 hidden class

### 修復
- 修復遷出和首站遷出按鈕同時顯示的問題
- 修復三個按鈕高度不一致的問題
- 修復操作者設定底部工作表提交按鈕超出邊界的問題
