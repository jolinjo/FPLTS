...
#### 3.0: 本站條碼（無論是否有記錄，都只允許查詢）

| 條件 | 行為 | 說明 |
|------|------|------|
| `barcode_process == current_station` | **只允許查詢** | 不能在本站掃到本站條碼時進行遷入或遷出 |

**邏輯**：
- **優先檢查**：如果條碼製程代號與當前站點相同，無論是否有記錄，都只允許查詢
- 這是為了防止在錯誤的站點進行操作（例如：P1 站點掃到 P1 條碼時，不應該再進行遷入或遷出）
- 自動開啟追溯查詢功能
- 自動填入條碼
- 提示：檢測到本站條碼，只能進行查詢
...
5. **本站條碼特殊處理**：
   - 如果條碼製程代號與當前站點相同（本站條碼），無論是否有記錄，都只允許查詢
   - 這是為了防止在錯誤的站點進行操作（例如：P1 站點掃到 P1 條碼時，不應該再進行遷入或遷出）
   - 系統會自動開啟追溯查詢功能，並顯示提示訊息

6. **異常情況處理**：
   - 下一站條碼在當前站點沒有記錄，可能是異常情況（跳站）
   - 系統只允許查詢，不允許遷入或遷出

## 更新記錄

- 2025-01-XX: 初始版本，定義所有狀態組合和行為邏輯
- 2025-01-XX: 更新本站條碼邏輯，無論是否有記錄都只允許查詢，防止在錯誤站點操作
