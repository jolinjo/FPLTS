...
2. **智能功能切換**：根據條碼狀態自動判斷應使用的功能（遷入/遷出/查詢）。
3. **權限管控**：強制工號登入。
...
### 3.4 `config/container.ini` & `config/status.ini`
...
**後端邏輯**：
1. 解析條碼，取得 SKU 與相關資訊。
2. 檢查該條碼在當前站點是否已有遷入記錄。
3. 檢查該條碼是否在其他站點已有遷入記錄（如有則不允許遷入）。
4. 寫入 Google Sheet (Action='IN')。
...
系統在掃描條碼時會自動檢查條碼狀態，並根據簡化的邏輯決定應該開啟哪個功能：
